...
if($_SERVER['REQUEST_METHOD']==='POST'){
    ...
    if($id){
        $stmt = $pdo->prepare("UPDATE customers SET name=?, phone=?, address=?, notes=? WHERE id=?");
...
<div class="d-flex justify-content-between align-items-center mb-4">
  <div>
    <h2 class="mb-1">
      <i class="fas fa-users me-2 text-primary"></i>Quản lý khách hàng
    </h2>
    <p class="text-muted mb-0">Quản lý danh sách khách hàng và thông tin liên hệ</p>
  </div>
  <div class="d-flex gap-2">
    <a class="btn btn-outline-primary" href="/?page=export&type=customers">
...
<div class="card shadow-sm">
  <div class="card-header">
    <h5 class="mb-0">
      <i class="fas fa-list me-2"></i>Danh sách khách hàng (<?= count($rows) ?> khách hàng)
    </h5>
  </div>
                ...
                <td class="text-center">
                  <div class="btn-group btn-group-sm">
                    ...
                    <a class="btn btn-outline-danger btn-delete" 
                       href="/?page=customers&action=delete&id=<?= $r['id'] ?>" 
        ...
        <div class="modal-body">
          <div class="row g-3">
            <div class="col-md-8">
              ...
              <input class="form-control" name="name" required placeholder="Nhập tên khách hàng">
            </div>
            <div class="col-md-4">
            ...
            <div class="col-12">
              ...
              <input class="form-control" name="address" placeholder="Nhập địa chỉ khách hàng">
            </div>
            <div class="col-12">
              <label class="form-label">
                <i class="fas fa-sticky-note me-1"></i>Ghi chú
              </label>
              <textarea class="form-control" name="notes" rows="3" placeholder="Thông tin bổ sung về khách hàng..."></textarea>
            </div>
          </div>
        </div>
...
document.getElementById("customerModal").addEventListener("show.bs.modal", e => {
  ...
  if(d){ 
    ...
    for(const k in o){
      const el = document.querySelector(`#customerForm [name=${k}]`); 
      if(el) el.value = o[k] ?? ''; 
    }

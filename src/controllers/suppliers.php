...
if($_SERVER['REQUEST_METHOD']==='POST'){
    ...
    if($id){
        $stmt=$pdo->prepare("UPDATE suppliers SET name=?, unit=?, sku=?, cost_hint=?, price_hint=?, notes=? WHERE id=?");
        $stmt->execute([$name,$unit,$sku,$cost,$price,$notes,$id]);
    } else {
        $stmt=$pdo->prepare("INSERT INTO suppliers(name,unit,sku,cost_hint,price_hint,notes) VALUES(?,?,?,?,?,?)");
        $stmt->execute([$name,$unit,$sku,$cost,$price,$notes]);
    }
    redirect('/?page=suppliers');
}
if(($_GET['action']??'')==='delete' && isset($_GET['id'])){
    $stmt=$pdo->prepare("DELETE FROM suppliers WHERE id=?"); $stmt->execute([(int)$_GET['id']]);
    redirect('/?page=suppliers');
}
...
<div class="d-flex justify-content-between align-items-center mb-3">
  ...
  <div class="d-flex gap-2">
    <a class="btn btn-secondary" href="/?page=export&type=suppliers">Xuất CSV</a>
      ...
      <?php foreach($rows as $r): ?>
        <tr>
          ...
          <td class="text-end">
            ...
            <a class="btn btn-sm btn-outline-danger" onclick="return confirm('Xóa?')" href="/?page=suppliers&action=delete&id=<?= $r['id'] ?>">Xóa</a>
          </td>
        </tr>
      <?php endforeach; ?>
    </tbody>
